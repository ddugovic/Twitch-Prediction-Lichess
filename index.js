  ...
  for await (const chunk of response.body) {
    ...
    if (chunk.length > 1) try {
      ...
      if (json.type == 'gameFinish' && game.id == gameId && prediction) {
        ...
        endPrediction(game.playerIndex, game.winner || game.status?.name);
        gameId = undefined;
...
async function createPrediction(game) {
  prediction = await api.predictions.createPrediction(broadcaster, {
    ...
    outcomes: [getName(user), getPlayerName(game.opponent), "Draw"],
...
async function endPrediction(playerIndex, outcome) {
